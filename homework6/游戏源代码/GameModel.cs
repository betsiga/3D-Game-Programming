...
public class GameModel : SSActionManager, ISSActionCallback {
    public GameObject PatrolItem, PlayerItem, sceneModelItem, canvasItem;

    private SceneController scene;
    private GameObject myPlayer, sceneModel, canvasAndText;
    ...
    protected new void Update() {
        PERSON_SPEED_NORMAL += 0.00002f;
        PERSON_SPEED_CATCHING += 0.00002f;
        base.Update();
    }

    //生产英雄
    void genPlayer() {
        myPlayer = Instantiate(PlayerItem);
    }
    ...
    public void heroMove(int dir) {
        myPlayer.transform.rotation = Quaternion.Euler(new Vector3(0, dir * 90, 0));
        switch (dir) {
            case Diretion.UP:
                myPlayer.transform.position += new Vector3(0, 0, 0.1f);
                break;
            case Diretion.DOWN:
                myPlayer.transform.position += new Vector3(0, 0, -0.1f);
                break;
            case Diretion.LEFT:
                myPlayer.transform.position += new Vector3(-0.1f, 0, 0);
                break;
            case Diretion.RIGHT:
                myPlayer.transform.position += new Vector3(0.1f, 0, 0);
                break;
    ...
    public void addDirectMovement(GameObject sourceObj) {
        ...
        Vector3 oriTarget = myPlayer.transform.position - sourceObj.transform.position;
    ...
    public int getPlayerStandOnArea() {
        return myPlayer.GetComponent<PlayerBehaviour>().standOnArea;
    }
}
